...
app.conf.update(
    ...
    CELERY_IMPORTS = ("addsendapp","preparesendapp","dosendapp","addJob","addreward","addsendmail","addsendmsg","addsendsms","addtask","custinfosync","custsync","doreward","dosendmail","dosendmsg","dosendsms","findJob","preparereward","preparesendmail","preparesendmsg","preparesendsms","sessionclose","tagsync","usercheck",)
)
...
@app.task(name="srjob.usertag.add")
def add(task):
    print(task)
    ...
    try:
        if not custids:
            ...
            if "tradbegintime" in customer.keys():
                conditions.append(" AND sc.id in (select custid from sr_cust_trade st where st.tradetime >= \"%s 00:00:00\")" % customer["tradbegintime"])
            if "tradendtime" in customer.keys():
                conditions.append(" AND sc.id in (select custid from sr_cust_trade st where st.tradetime <= \"%s 23:59:59\")" % customer["tradendtime"])
            ...
            # 倒计数
            # 一并加上查询到的数量
            redisdb.hmset("task:"+str(task_id),{"queried": count, "downcount": count})
            #创建子任务
            #最后创建子任务，易于处理downcount
            #for x in range((count + ONCE_CAPACITY -1) /ONCE_CAPACITY):
            if count:
                app.send_task('srjob.usertag.usercheck',args=[task_id, userid, tagid,0],queue='usercheck')
            else:
            ...
            # 倒计数
            # 一并加上查询到的数量
            redisdb.hmset("task:"+str(task_id),{"queried": len(custids), "downcount": len(custids)})
            #创建子任务
            #最后创建子任务，易于处理downcount
            #for x in range((len(custids) + ONCE_CAPACITY -1) /ONCE_CAPACITY):
            if count:
                app.send_task('srjob.usertag.usercheck',args=[task_id, userid.encode('utf-8'), tagid,0],queue='usercheck')
...
if __name__ == "__main__":
    ...
    # 应确保全局只有一份同样的beat
    app.worker_main(["worker","--loglevel=debug","-n","tasks.%h"])
