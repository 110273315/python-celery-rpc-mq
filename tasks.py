...
def add(task):
    ...
    try:
        if not custids:
            ...
            # 倒计数
            # 一并加上查询到的数量
            mongo["task"].update_one({"_id": task_id}, {"$set": {"queried": len(custids), "downcount": len(custids)}})
            #创建子任务
            #最后创建子任务，易于处理downcount
            for x in range((len(custids) + ONCE_CAPACITY -1) /ONCE_CAPACITY):
                ...
                print(userid)
                app.send_task('srjob.usertag.usercheck',args=[task_id, userid.encode('utf-8'), tagid],queue='usercheck')
...
if __name__ == "__main__":
    ...
    # 应确保全局只有一份同样的beat
    app.worker_main(["worker", "--beat", "--loglevel=debug","--concurrency=10","-n","tasks.%h"])
